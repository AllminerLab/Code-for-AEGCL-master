...
def get_dataset(path, name):
    assert name in ['Cora', 'CiteSeer', 'PubMed', 'CoraFull', 'DBLP', 'Karate', 'WikiCS', 'Coauthor-CS', 'Coauthor-Phy',
    ...
    if name == 'CoraFull':
        return CitationFull(root=path, name="Cora", transform=T.NormalizeFeatures())
    
    if name == 'DBLP':
        return CitationFull(root=path, name="DBLP", transform=T.NormalizeFeatures())
