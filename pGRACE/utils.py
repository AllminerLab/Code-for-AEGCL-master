...
class Up_proj(nn.Module):
    def __init__(self, in_dim, out_dim):
        super().__init__()
        self.up_proj = nn.Linear(in_dim, out_dim, bias=True)
        nn.init.xavier_normal_(self.up_proj.weight, gain=1.414)
    
    def forward(self, emb):
        return self.up_proj(emb)

def motif_drop_weights(col):
    s_col = torch.log(col)
